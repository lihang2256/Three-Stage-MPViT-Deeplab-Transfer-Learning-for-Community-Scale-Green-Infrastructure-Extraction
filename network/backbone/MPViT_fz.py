...
model_urls = {
    'mpvit_base': 'C:/Users/37161/.cache/torch/hub/checkpoints/best_deeplabv3plus_mpvit_land3_os16.pth'
}
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed1 = Patch_Embed_stage(
            embed_dims[0],
            num_path=num_path[0],
            isPool=False,
        )

        self.mhca1 = MHCA_stage(
            embed_dims[0],
            embed_dims[1],
            num_layers[0],
            num_heads[0],
        ...
        self.patch_embed2 = Patch_Embed_stage(
            embed_dims[1],
            num_path=num_path[1],
            isPool=True,
        )

        self.mhca2 = MHCA_stage(
            embed_dims[1],
            embed_dims[2],
            num_layers[1],
            num_heads[1],
        ...
        self.patch_embed3 = Patch_Embed_stage(
            embed_dims[2],
            num_path=num_path[2],
            isPool=True,
        )

        self.mhca3 = MHCA_stage(
            embed_dims[2],
            embed_dims[3],
            num_layers[2],
            num_heads[2],
        ...
        self.patch_embed4 = Patch_Embed_stage(
            embed_dims[3],
            num_path=num_path[3],
            isPool=True,
        )

        self.mhca4 = MHCA_stage(
            embed_dims[3],
            embed_dims[3],
            num_layers[3],
            num_heads[3],
    ...
    def forward_features(self, x):
        ...
        #     x = self.patch_embed_mhca_stages[i](x)
...
@register_model
# deeplabv3+ backbone
def mpvit_fz_base(pretrained=True, progress=True, output_stride=8, **kwargs):
    """mpvit_fz_base :
    ...
    if pretrained:
        pretrained_dict = load_state_dict_from_url(model_urls['mpvit_base'],
                                                   progress=progress)
        if model_urls['mpvit_base'] == 'C:/Users/37161/.cache/torch/hub/checkpoints/mpvit_base.pth':
            print('base pretrain')
            ############################## orderdict 数据迁移 #################################################
            # print(list(pretrained_dict['model'].keys()))
            ...
            for item in model_dict.keys():
                arr = item.split('.')
                if arr[0][0:3] == "cls":
                    update_dict[item] = pretrained_dict['model'][item]
                elif arr[0][0:3] == "ste":
                    s = 'stem.' + str(int(arr[0][4]) - 1) + '.'
                    for i in range(1, len(arr) - 1):
                        s = s + arr[i] + '.'
                    s = s + arr[len(arr) - 1]
                    update_dict[item] = pretrained_dict['model'][s]
                elif arr[0][0:3] == "pat":
                    s = 'patch_embed_stages.' + str(int(arr[0][11]) - 1) + '.'
                    for i in range(1, len(arr) - 1):
                        s = s + arr[i] + '.'
                    s = s + arr[len(arr) - 1]
                    update_dict[item] = pretrained_dict['model'][s]
                elif arr[0][0:3] == "mhc":
                    s = 'mhca_stages.' + str(int(arr[0][4]) - 1) + '.'
                    for i in range(1, len(arr) - 1):
                        s = s + arr[i] + '.'
                    s = s + arr[len(arr) - 1]
                    update_dict[item] = pretrained_dict['model'][s]
            ...
            print('trans pretrain')
            # print(list(pretrained_dict['model_state'].keys()))
            model_dict = model.state_dict()
            # print(list(model_dict.keys()))
            for item in model_dict.keys():
                s = 'backbone.' + item
                if item[0:3] != 'cls':
                    model_dict[item] = pretrained_dict['model_state'][s]
                else:
                    break
